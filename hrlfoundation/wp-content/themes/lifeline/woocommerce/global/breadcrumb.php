...
if ( (!is_home() && !is_front_page() && !( is_post_type_archive() && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_category() ) {
		...
		echo $prepend . $before . __( 'Products tagged &ldquo;', SH_NAME ) . $queried_object->name . '&rdquo;' . $after;
	} elseif ( is_day() ) {
		...
		if ( is_search() ) {
			echo $before . '<a href="' . get_post_type_archive_link( 'product' ) . '">' . $_name . '</a>' . $delimiter . __( 'Search results for &ldquo;', SH_NAME ) . get_search_query() . '&rdquo;' . $after;
		} elseif ( is_paged() ) {
		...
		echo $before . __( 'Error 404', SH_NAME ) . $after;
		...
		echo $before . __( 'Search results for &ldquo;', SH_NAME ) . get_search_query() . '&rdquo;' . $after;
	} elseif ( is_tag() ) {
		echo $before . __( 'Posts tagged &ldquo;', SH_NAME ) . single_tag_title( '', false ) . '&rdquo;' . $after;
	} elseif ( is_author() ) {
		$userdata = get_userdata( $author );
		echo $before . __( 'Author:', SH_NAME ) . ' ' . $userdata->display_name . $after;
	}
	if ( get_query_var( 'paged' ) )
		echo ' (' . __( 'Page', SH_NAME ) . ' ' . get_query_var( 'paged' ) . ')';
	echo $wrap_after;
}
