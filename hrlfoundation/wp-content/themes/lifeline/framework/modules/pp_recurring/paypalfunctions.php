...
//'------------------------------------
	var $API_UserName = "shahbaz_ahmed98-facilitator_api1.yahoo.com";
	var $API_Password = "1378028835";
	var $API_Signature = "AKRKg7WOTdG07scdT9XjBbw3pATmA4wSsfplC-53DiEYe.3qxlnwkT5p";
// BN Code 	is only applicable for partners
	...
	function __construct() {
		$donation_data = get_option( SH_NAME ); //printr($donation_data);
		$this->SandboxFlag = sh_set( $donation_data, 'paypal_type' );
		$this->API_UserName = sh_set( $donation_data, 'paypal_api_username' );
		$this->API_Password = sh_set( $donation_data, 'paypal_api_password' );
		$this->API_Signature = sh_set( $donation_data, 'paypal_api_signature' );
...
// Construct the parameter string that describes the SetExpressCheckout API call in the shortcut implementation
		...
		$nvpstr = $nvpstr . "&BILLINGAGREEMENTDESCRIPTION=" . urlencode( "Donation" );
...
//'---------------------------------------------------------------------------------------------------------------
		...
		$resArray = $this->hash_call( "SetExpressCheckout", $nvpstr ); //printr($resArray);
...
//Format the other parameters that were stored in the session from the previous calls
		...
		$_SESSION['billing_agreemenet_id'] = sh_set( $resArray, "BILLINGAGREEMENTID" );
...
//'---------------------------------------------------------------------------
		...
		$nvpstr.="&PROFILESTARTDATE=" . urlencode( date( 'c' ) );
		$nvpstr.="&DESC=" . urlencode( "Donation" );
		$nvpstr.="&BILLINGPERIOD=" . $_SESSION['Billing_Period'];
		$nvpstr.="&BILLINGFREQUENCY=" . $_SESSION['Billing_Frequency'];
		$nvpstr.="&AMT=" . $_SESSION['Payment_Amount'];
		$nvpstr.="&CURRENCYCODE=" . $_SESSION["Currency_Code"];
