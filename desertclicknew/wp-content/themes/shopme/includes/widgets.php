...
if (!function_exists('shopme_add_to_mailchimp_list')) {
	...
	function shopme_add_to_mailchimp_list() {
		...
		$result['text']  = esc_html__('You\'ve been added to our sign-up list. We have sent an email, asking you to confirm the same.', 'shopme');
			...
			<ul class="c_info_list">
				...
				<?php if (!empty($email)): ?>
					<li class="c_info_mail">
						<a target="_blank" class="over" href="mailto:<?php echo esc_url($email) ?>"><?php echo $email ?></a>
					</li>
				<?php endif; ?>
		...
		function widget($args, $instance) {
			...
			<div class="<?php echo esc_attr($type) ?>">
				...
				<?php foreach ($this->entries->posts as $entry):
						...
						$place = rwmb_meta('shopme_tm_place', '', $id);
					?>
					<blockquote>
