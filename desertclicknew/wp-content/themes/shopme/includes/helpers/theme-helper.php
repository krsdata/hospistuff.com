	...
	class SHOPME_HELPER {
		...
		public static function check_animate_widgets($post_id) {
			...
			if (shopme_custom_get_option('animate_widgets_pages')) {
				$result = rwmb_meta('shopme_animate_widgets', '', $post_id);
			}
		...
		public static function check_page_layout ($post_id = false) {
			...
			if (is_singular()) {
				$result = rwmb_meta('shopme_page_sidebar_position', '', $post_id);
			}
			if (is_404()) {
				$result = 'no_sidebar';
			}
			...
			if (shopme_is_shop_installed()) {
				...
				if (shopme_is_product()) {
					$result_sidebar_position = rwmb_meta('shopme_page_sidebar_position', '', $post_id);
		...
		public static function page_layout () {
			...
			@$page_layout = rwmb_meta('shopme_page_layout', '', $post_id);
							...
							<?php if ($entries == '') : ?>
								...
								<?php
									...
									if ( 1 == $total ) {
										...
										printf( esc_html__( 'Showing all %d results', 'shopme' ), $total );
									} else {
										printf( esc_html__( 'Showing %d &ndash; %d of %d results', 'shopme' ), $first, $last, $total );
									}
								?>
