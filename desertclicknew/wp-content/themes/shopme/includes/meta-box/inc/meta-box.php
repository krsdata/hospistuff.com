...
if ( ! class_exists( 'RW_Meta_Box' ) )
{
	...
	class RW_Meta_Box
	{
		...
		function admin_enqueue_scripts()
		{
			...
			wp_enqueue_style( SHOPME_PREFIX . 'rwmb', RWMB_CSS_URL . 'style.css', array(), RWMB_VER );
			...
			if ( $has_clone )
				wp_enqueue_script( SHOPME_PREFIX . 'rwmb-clone', RWMB_JS_URL . 'clone.js', array( 'jquery' ), RWMB_VER, true );

			if ( $this->validation )
			{
				wp_enqueue_script( SHOPME_PREFIX . 'jquery-validate', RWMB_JS_URL . 'jquery.validate.min.js', array( 'jquery' ), RWMB_VER, true );
				wp_enqueue_script( SHOPME_PREFIX . 'rwmb-validate', RWMB_JS_URL . 'validate.js', array( SHOPME_PREFIX . 'jquery-validate' ), RWMB_VER, true );
			}

			// Auto save
			if ( $this->meta_box['autosave'] )
				wp_enqueue_script( SHOPME_PREFIX . 'rwmb-autosave', RWMB_JS_URL . 'autosave.js', array( 'jquery' ), RWMB_VER, true );
		}
				...
				echo '
					...
					if ( typeof rwmb == "undefined" )
					{
						var rwmb = {
							...
							summaryMessage : "' . esc_js( __( 'Please correct the errors highlighted below and try again.', 'shopme' ) ) . '"
						};
					}
					else
					{
