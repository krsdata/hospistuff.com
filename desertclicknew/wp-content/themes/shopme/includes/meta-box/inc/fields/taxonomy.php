...
if ( ! class_exists( 'RWMB_Taxonomy_Field' ) )
{
	class RWMB_Taxonomy_Field extends RWMB_Field
	{
		...
		static function admin_enqueue_scripts()
		{
			...
			wp_enqueue_style( SHOPME_PREFIX . 'rwmb-taxonomy', RWMB_CSS_URL . 'taxonomy.css', array(), RWMB_VER );
			wp_enqueue_script( SHOPME_PREFIX . 'rwmb-taxonomy', RWMB_JS_URL . 'taxonomy.js', array( 'jquery', SHOPME_PREFIX . 'rwmb-select-advanced', 'wp-ajax-response' ), RWMB_VER, true );
		}
		...
		static function normalize_field( $field )
		{
			...
			$field['placeholder'] = empty( $field['placeholder'] ) ? sprintf( esc_html__( 'Select a %s' , 'shopme' ), $tax->labels->singular_name ) : $field['placeholder'];
