...
if ( ! class_exists( 'RWMB_Map_Field' ) )
{
	class RWMB_Map_Field extends RWMB_Field
	{
		...
		{
			wp_enqueue_script( SHOPME_PREFIX . 'googlemap', 'https://maps.google.com/maps/api/js?sensor=false', array(), '', true );
			wp_enqueue_script( SHOPME_PREFIX . 'rwmb-map', RWMB_JS_URL . 'map.js', array( 'jquery', 'jquery-ui-autocomplete', SHOPME_PREFIX . 'googlemap' ), RWMB_VER, true );
		}
		...
		static function html( $meta, $field )
		{
			...
			if ( $address )
			{
				$html .= sprintf(
					...
					esc_html__( 'Find Address', 'shopme' )
				);
			}
