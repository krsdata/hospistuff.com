...
if ( ! class_exists( 'RWMB_Image_Advanced_Field' ) )
{
	class RWMB_Image_Advanced_Field extends RWMB_Image_Field
	{
		...
		static function admin_enqueue_scripts()
		{
			...
			wp_enqueue_media();
			wp_enqueue_script( SHOPME_PREFIX . 'rwmb-image-advanced', RWMB_JS_URL . 'image-advanced.js', array( 'jquery', 'underscore' ), RWMB_VER, true );
			wp_localize_script( SHOPME_PREFIX . 'rwmb-image-advanced', 'rwmbImageAdvanced', array(
				'frameTitle' => esc_html__( 'Select Images', 'shopme' ),
			) );
		}
		...
		{
			$i18n_title = apply_filters( 'rwmb_image_advanced_select_string', _x( 'Select or Upload Images', 'image upload', 'shopme' ), $field );
