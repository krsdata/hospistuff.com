...
if ( ! class_exists( 'RWMB_File_Field' ) )
{
	class RWMB_File_Field extends RWMB_Field
	{
		...
		{
			wp_enqueue_style( SHOPME_PREFIX . 'rwmb-file', RWMB_CSS_URL . 'file.css', array(), RWMB_VER );
			wp_enqueue_script( SHOPME_PREFIX . 'rwmb-file', RWMB_JS_URL . 'file.js', array( 'jquery' ), RWMB_VER, true );
			wp_localize_script( SHOPME_PREFIX . 'rwmb-file', 'rwmbFile', array(
				'maxFileUploadsSingle' => esc_html__( 'You may only upload maximum %d file', 'shopme' ),
				'maxFileUploadsPlural' => esc_html__( 'You may only upload maximum %d files', 'shopme' ),
			) );
		}
		...
		static function wp_ajax_delete_file()
		{
			...
			if ( $ok )
				...
				wp_send_json_error( esc_html__( 'Error: Cannot delete file', 'shopme' ) );
		}
		...
		{
			$i18n_title = apply_filters( 'rwmb_file_upload_string', _x( 'Upload Files', 'file upload', 'shopme' ), $field );
			$i18n_more  = apply_filters( 'rwmb_file_add_string', _x( '+ Add new file', 'file upload', 'shopme' ), $field );
		...
		{
			$i18n_delete = apply_filters( 'rwmb_file_delete_string', _x( 'Delete', 'file upload', 'shopme' ) );
			$i18n_edit   = apply_filters( 'rwmb_file_edit_string', _x( 'Edit', 'file upload', 'shopme' ) );
			$li = '
			<li>
