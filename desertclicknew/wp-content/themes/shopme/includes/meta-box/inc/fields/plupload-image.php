...
if ( ! class_exists( 'RWMB_Plupload_Image_Field' ) )
{
	class RWMB_Plupload_Image_Field extends RWMB_Image_Field
	{
		...
		static function admin_enqueue_scripts()
		{
			...
			wp_enqueue_style( SHOPME_PREFIX . 'rwmb-plupload-image', RWMB_CSS_URL . 'plupload-image.css', array( 'wp-admin' ), RWMB_VER );
			wp_enqueue_script( SHOPME_PREFIX . 'rwmb-plupload-image', RWMB_JS_URL . 'plupload-image.js', array( 'jquery-ui-sortable', 'wp-ajax-response', 'plupload-all' ), RWMB_VER, true );
			wp_localize_script( SHOPME_PREFIX . 'rwmb-plupload-image', 'RWMB', array( 'url' => RWMB_URL ) );
		}
		...
		static function html( $meta, $field )
		{
			...
			$i18n_drop   = apply_filters( 'rwmb_plupload_image_drop_string', _x( 'Drop images here', 'image upload', 'shopme' ), $field );
			$i18n_or     = apply_filters( 'rwmb_plupload_image_or_string', _x( 'or', 'image upload', 'shopme' ), $field );
			$i18n_select = apply_filters( 'rwmb_plupload_image_select_string', _x( 'Select Files', 'image upload', 'shopme' ), $field );
		...
		static function normalize_field( $field )
		{
			$field['js_options'] = array(
				...
				'filters'				=> array(
					array(
						'title'      => _x( 'Allowed Image Files', 'image upload', 'shopme' ),
