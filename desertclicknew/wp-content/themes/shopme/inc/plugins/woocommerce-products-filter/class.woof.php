...
//			'by_insales'
		...
		public static $query_cache_table = 'shopme_woof_query_cache';
		private $session_rct_key = 'shopme_woof_really_current_term';
		...
		public function __construct() {
			...
			$this->storage = new SHOPME_WOOF_STORAGE($this->storage_type);
		...
		public function init() {
			...
			$first_init = (int) get_option('shopme_woof_first_init');
			if ($first_init != 1) {
				update_option('shopme_woof_first_init', 1);
				update_option('shopme_woof_show_count', 1);
				update_option('shopme_woof_checkboxes_slide', 1);
				update_option('shopme_woof_show_reset', 1);
				update_option('shopme_behavior', 'title');
			}

			add_action('woocommerce_settings_tabs_array', array($this, 'woocommerce_settings_tabs_array'), 50);
			add_action('woocommerce_settings_tabs_shopme_woof', array($this, 'print_plugin_options'), 50);

			add_action('wp_enqueue_scripts', array($this, 'add_enqueue_scripts'), 1);

			add_action('wp_head', array($this, 'wp_head'), 999);

			add_filter('shopme_woof_get_request_data', array($this, 'woof_get_request_data'), 1);
			...
			add_shortcode('shopme_woof', array($this, 'woof_shortcode'));
			add_shortcode('shopme_woof_products', array($this, 'woof_products'));
			add_shortcode('shopme_woof_sku_filter', array($this, 'woof_sku_filter'));
			add_shortcode('shopme_woof_text_filter', array($this, 'woof_text_filter'));
			...
			add_action('wp_ajax_shopme_woof_select_type', array( $this, 'ajax_print_terms') );
	}

		public function registerWidgets() {
			register_widget('shopme_woof_widget');
		}
		...
		public function ajax_print_terms() {
			...
			check_ajax_referer('shopme_woof_select_type');
		...
		public function woocommerce_settings_tabs_array($tabs) {
			$tabs['shopme_woof'] = esc_html__('Products Filter', 'shopme');
			return $tabs;
		}
		...
		public function add_enqueue_scripts() {
			...
			if ( ! is_admin() ) {
				...
				wp_enqueue_style( SHOPME_PREFIX . 'woof', SHOPME_WOOF_LINK . 'css/woof-front.css' );
				wp_enqueue_script( SHOPME_PREFIX . 'woof', SHOPME_WOOF_LINK . 'js/woof-front.js', array( 'jquery', SHOPME_PREFIX . 'woocommerce-mod' ), WC_VERSION, true );
			}
		...
		public function woof_shortcode($atts) {
			...
			if (!empty($taxonomies)) {
				foreach ($taxonomies as $tax_key => $tax) {
					$args['shopme_woof_settings'] = get_option('shopme_woof_settings');
					$args['taxonomies_info'][$tax_key] = $tax;
					$args['taxonomies'][$tax_key] = SHOPME_WOOF_HELPER::get_terms($tax_key);
				}
			}
			...
			return $this->render_html( SHOPME_WOOF_PATH . 'views/woof.php', $args );
		}

		public function woof_sku_filter($args = array()) {
			return $this->render_html( SHOPME_WOOF_PATH . 'views/shortcodes/woof_sku_filter.php', $args );
		}

		public function woof_text_filter($args = array()) {
			return $this->render_html( SHOPME_WOOF_PATH . 'views/shortcodes/woof_text_filter.php', $args );
		}
		...
		public function print_plugin_options() {
			...
			wp_enqueue_script( SHOPME_PREFIX . 'woof', SHOPME_WOOF_LINK . 'js/plugin_options.js', array( 'jquery' ) );
			wp_enqueue_style( SHOPME_PREFIX . 'woof', SHOPME_WOOF_LINK . 'css/plugin_options.css' );

			if (isset($_POST['shopme_woof_settings'])) {

				WC_Admin_Settings::save_fields($this->get_options());

				if (class_exists('SitePress')) {
					$lang = ICL_LANGUAGE_CODE;
					if (isset($_POST['shopme_woof_settings']['wpml_tax_labels']) AND ! empty($_POST['shopme_woof_settings']['wpml_tax_labels'])) {
						$translations_string = $_POST['shopme_woof_settings']['wpml_tax_labels'];
						...
						$_POST['shopme_woof_settings']['wpml_tax_labels'] = $translations;
					}
				}
				update_option('shopme_woof_settings', $_POST['shopme_woof_settings']);
				$this->init_settings();
			}

			$args = array("shopme_woof_settings" => get_option('shopme_woof_settings'));
			echo $this->render_html(SHOPME_WOOF_PATH . 'views/plugin_options.php', $args);
		}

		private function init_settings() {
			$this->settings = get_option('shopme_woof_settings', array());
		}
		...
		public function wp_head() {
			...
			<script type="text/javascript">
				...
				var shopme_woof_is_permalink = <?php echo intval((bool) $this->is_permalink_activated()) ?>;

				var shopme_woof_shop_page = "";
				<?php if (!$this->is_permalink_activated()): ?>
					shopme_woof_shop_page = "<?php echo home_url('/?post_type=product'); ?>";
				<?php endif; ?>

				var shopme_woof_really_curr_tax = {};
				var shopme_swoof_search_slug = "<?php echo $this->get_swoof_search_slug(); ?>";
				var shopme_woof_current_page_link = location.protocol + '//' + location.host + location.pathname;
					...
					if (!isset($wp_query->query_vars['taxonomy'])) {
						...
						if ( is_string($link) AND ! empty($link) ) {
							?> shopme_woof_current_page_link = "<?php echo $link ?>"; <?php
						}
					}
					...
					if (!defined('DOING_AJAX') AND ! is_page()) {
						...
						if (isset($wp_query->query_vars['taxonomy']) AND empty($request_data)) {
							...
							if (is_object($queried_obj)) {
								...
								?>
								shopme_woof_really_curr_tax = {term_id:<?php echo $queried_obj->term_id ?>, taxonomy: "<?php echo $queried_obj->taxonomy ?>"};
								<?php
							}
				...
				?>
					var shopme_woof_current_values = '<?php echo json_encode($this->get_request_data()); ?>',
						shopme_woof_ajaxurl = "<?php echo admin_url('admin-ajax.php'); ?>",
						shopme_woof_ajax_page_num =<?php echo ((get_query_var('page')) ? get_query_var('page') : 1) ?>,
						shopme_woof_ajax_per_page = <?php echo shopme_custom_get_option('woocommerce_product_count') ?>,
					...
					<?php if (isset($request_data['really_curr_tax'])): ?>
						...
						?>
						shopme_woof_really_curr_tax = {term_id:<?php echo $tmp[0] ?>, taxonomy: "<?php echo $tmp[1] ?>"};
					<?php endif; ?>
		...
		public function get_options() {
			$options = array(
				array(
					'name' => esc_html__('Products Filter Options', 'shopme'),
					'type' => 'title',
					'desc' => '',
					'id' => 'shopme_woof_general_settings'
				),
				array(
					'name' => __('Hide childs in checkboxes', 'shopme'),
					'desc' => __('Hide childs in checkboxes. Near checkbox which has childs will be plus icon to show childs.', 'shopme'),
					'id' => 'shopme_woof_checkboxes_slide',
					'type' => 'select',
					'class' => 'chosen_select',
					'css' => 'min-width:300px;',
					'options' => array(
				...
				array(
					'name' => __('Show count', 'shopme'),
					'desc' => __('Show count of items near taxonomies terms on the front', 'shopme'),
					'id' => 'shopme_woof_show_count',
					'type' => 'select',
					'class' => 'chosen_select',
					'css' => 'min-width:300px;',
					'options' => array(
				...
				array(
					'name' => esc_html__('Show reset', 'shopme'),
					'desc' => esc_html__('Show reset products filter', 'shopme'),
					'id' => 'shopme_woof_show_reset',
					'type' => 'select',
					'class' => 'chosen_select',
					'css' => 'min-width:300px;',
					'options' => array(
						0 => esc_html__('No', 'shopme'),
						1 => esc_html__('Yes', 'shopme')
					),
					'default'  => 1,
					'desc_tip' => true
				),
				array(
					'name' => esc_html__('Behavior', 'shopme'),
					'desc' => esc_html__('Behaviour of the text searching', 'shopme'),
					'id' => 'shopme_behavior',
					'type' => 'select',
					'class' => 'chosen_select',
					'css' => 'min-width:300px;',
					'options' => array(
						'title' => esc_html__("Search by title", 'shopme'),
						'content' => esc_html__("Search by content", 'shopme'),
						'excerpt' => esc_html__("Search by excerpt", 'shopme'),
						'content_or_excerpt' => esc_html__("Search by content OR excerpt", 'shopme'),
						'title_or_content_or_excerpt' => esc_html__("Search by title OR content OR excerpt", 'shopme'),
						'title_or_content' => esc_html__("Search by title OR content", 'shopme'),
						'title_and_content' => esc_html__("Search by title AND content", 'shopme')
					),
					'default'  => 'title',
					'desc_tip' => true
				),
				array('type' => 'sectionend', 'id' => 'shopme_woof_general_settings')
			);
			return apply_filters('wc_settings_tab_shopme_woof_settings', $options);
		}
		...
		public function dynamic_count($curr_term, $type, $additional_taxes = '') {
			...
			$query = new SHOPME_WP_QueryWoofCounter($args);
		...
		public static function attributes_table( $type, $attribute, $values = array(), $echo = true ) {
			...
			if ('color' == $type) {
				if (!empty($terms)) {
					$return = sprintf( '<table><tr><th>%s</th><th>%s</th></tr>', esc_html__( 'Term', 'shopme' ), esc_html__( 'Color', 'shopme' ) );
					foreach ( $terms as $term ) {
						$return .= "<tr><td><label for='{$attribute}_{$term->slug}'>{$term->name}</label></td><td><input type='text' id='{$attribute}_{$term->slug}' name='shopme_woof_settings[colors][{$attribute}][{$term->slug}]' value='" . ( isset( $values[$term->slug] ) ? $values[$term->slug] : '' ) . "' size='3' class='shopme-colorpicker' /></td></tr>";
					}
					$return .= '</table>';
				}
			} else if ('label' == $type) {
				if (!empty($terms)) {
					$return = sprintf( '<table><tr><th>%s</th><th>%s</th></tr>', esc_html__( 'Term', 'shopme' ), esc_html__( 'Label', 'shopme' ) );
					foreach ( $terms as $term ) {
						$return .= "<tr><td><label for='{$attribute}_{$term->slug}'>{$term->name}</label></td><td><input type='text' id='{$attribute}_{$term->slug}' name='shopme_woof_settings[labels][{$attribute}][{$term->slug}]' value='" . ( isset( $values[$term->slug] ) ? $values[$term->slug] : '' ) . "' size='3' /></td></tr>";
					}
					$return .= '</table>';
				}
			}
		...
		private function assemble_sku_params(&$meta_query) {
			...
			if (isset($request['shopme_woof_sku'])) {
				if (!empty($request['shopme_woof_sku'])) {
					$meta_query[] = array(
						'key' => '_sku',
						'value' => $request['shopme_woof_sku']
					);
				}
			}
		...
		public function woocommerce_before_shop_loop() {
			echo '<div class="woof_shortcode_output">';
				...
				$shortcode_txt = "shopme_woof_products is_ajax=1";

				if ($this->is_really_current_term_exists()) {
					$o = $this->get_really_current_term();
					$shortcode_txt = "shopme_woof_products taxonomies={$o->taxonomy}:{$o->term_id} is_ajax=1";
		...
		public function get_request_data() {
			return apply_filters('shopme_woof_get_request_data', $_GET);
		}

		public function woof_get_request_data($request) {
			if (isset($request['s'])) {
				$request['shopme_woof_text'] = $request['s'];
			}
			return $request;
		}
		...
		public function get_catalog_orderby($orderby = '', $order = 'ASC', $meta_key = '') {
...
//					$order = 'ASC';
		...
		public function woof_post_text_filter($where = '') {
			...
			if ($this->is_isset_in_request_data('shopme_woof_text') && !empty($request['shopme_woof_text'])) {

				$woof_text = strtolower($request['shopme_woof_text']);
				$behaviour = get_option('shopme_behavior');
		...
		public function woof_products($atts) {
			...
			$shortcode_txt = 'shopme_woof_products';
			...
			$data = $this->get_request_data();
			$catalog_orderby = $this->get_catalog_orderby(isset($data['orderby']) ? $data['orderby'] : '', isset($data['product_sort']) ? $data['product_sort'] : 'ASC');
			...
			$args['posts_per_page'] = shopme_custom_get_option('woocommerce_product_count');
...
//				add_filter('post_class', array($this, 'woo_post_class'));
		...
		public function woof_draw_products() {
			...
			parse_str($link, $_GET);
			$products = do_shortcode("[" . $_REQUEST['shortcode'] . " order=". $_REQUEST['order'] ." per_page=". $_REQUEST['per_page'] ." page=" . $_REQUEST['page'] . " ]");
			$form = '';
