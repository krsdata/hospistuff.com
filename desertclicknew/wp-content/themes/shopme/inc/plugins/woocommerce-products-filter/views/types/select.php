...
if (!function_exists('woof_draw_select_childs'))
{
	...
	function woof_draw_select_childs(&$collector, $taxonomy_info, $tax_slug, $childs, $level, $show_count)
	{
		...
		if (!in_array($term['slug'], $current_request)) {
			...
			if ($show_count) {
				$count = $SHOPME_WOOF->dynamic_count($term, 'select', $_REQUEST['additional_taxes']);
				$count_string = ' (' . $count . ')';
			}
		...
		if (!empty($term['childs'])) {
			woof_draw_select_childs($collector, $taxonomy_info, $tax_slug, $term['childs'], $level + 1, $show_count);
		}
		?>
	<?php endforeach; ?>
		<?php
...
?>
	<select class="shopme_woof_select" name="<?php echo $tax_slug ?>">

		<option value="0"><?php echo SHOPME_WOOF_HELPER::wpml_translate($taxonomy_info) ?></option>
