...
final class SHOPME_WOOF_HELPER {

	public static function is_active_widget_displayed() {
		if ( is_active_widget( false, false, 'shopme-widget-woof-filter', true ) ) {
			return true;
		} else {
			return false;
		}
	...
	public static function wpml_translate($taxonomy_info, $string = '') {
		...
		if (class_exists('SitePress')) {
			...
			$woof_settings = get_option('shopme_woof_settings');
		...
		if ($check_for_custom_label) {
			global $SHOPME_WOOF;
			if (isset($SHOPME_WOOF->settings['custom_tax_label'][$taxonomy_info->name]) AND ! empty($SHOPME_WOOF->settings['custom_tax_label'][$taxonomy_info->name]))
			{
				$string = $SHOPME_WOOF->settings['custom_tax_label'][$taxonomy_info->name];
			}
		}
	...
	{
		global $_chosen_attributes, $wpdb, $wp, $SHOPME_WOOF;
		...
		echo '<form method="get" action="' . esc_url($form_action) . '">
			<div class="price_slider_wrapper">
				...
				<div class="price_slider_amount">
					<input type="text" id="min_price" name="min_price" value="' . esc_attr($min_price) . '" data-min="' . esc_attr(apply_filters('woocommerce_price_filter_widget_amount', $min)) . '" placeholder="' . __('Min price', 'shopme') . '" />
					<input type="text" id="max_price" name="max_price" value="' . esc_attr($max_price) . '" data-max="' . esc_attr(apply_filters('woocommerce_price_filter_widget_amount', $max)) . '" placeholder="' . __('Max price', 'shopme') . '" />
					<button type="submit" class="button">' . __('Filter', 'shopme') . '</button>
					<div class="price_label" style="display:none;">
						' . __('Price:', 'shopme') . ' <span class="from"></span> &mdash; <span class="to"></span>
					</div>
					' . $fields . '
					<div class="clear"></div>
				</div>
